...
if (args.Length != 3)
{
    ...
    Console.Error.WriteLine("    Nerv.DearImGui.Generator <path-to-dear-imgui-source> <path-to-nerv-dearimgui-native> <path-to-output>");
    return 1;
}
...
if (OperatingSystem.IsWindows())
{
    ...
    importLibraryName = "Nerv.DearImGui.Native.lib";
...
string imguiLibFilePath = Path.Combine(dearImGuiNativeRootPath, "..", "bin", "Nerv.DearImGui.Native", dotNetRid, canonicalBuildVariant, importLibraryName);
...
library = new NervDearImGuiNamespaceTransformation().Transform(library);
...
(
    (c, d) => d.Namespace == "Nerv.DearImGui" ? "ImGui" : d.Namespace == "Nerv.DearImGui.Internal" ? "ImGuiInternal" : "Globals"
).Transform(library);
...
library = new OrganizeOutputFilesByNamespaceTransformation("Nerv.DearImGui").Transform(library); // Relies on MochiDearImGuiNamespaceTransformation, MoveLooseDeclarationsIntoTypesTransformation
...
Console.WriteLine("Rebuilding Nerv.DearImGui.Native...");
...
(
    ...
    {
        TargetRuntime = TargetRuntime.Net6,
        InfrastructureTypesNamespace = "Nerv.DearImGui.Infrastructure",
    },
    outputSession,
    library
);
